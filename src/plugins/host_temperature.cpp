...
This is the temperature plugin, enabling to estimate the temperature of a host and its surrounding air.
To activate this plugin, first call sg_host_temperature_plugin_init() before loading your platform.
If you are using master and slave hosts (in the sense of this temperature plugin), you have to call
sg_host_init_master_slaves() on every master and slave host after platform loading to complete the initialization.

During simulation, call sg_host_get_host_temperature() to retrieve the temperature of a given host.
It is also possible to retrieve the ambient air temperature with sg_host_get_air_temperature().
...
void HostTemperature::update()
{
	...
	// Get the new energy of me and all my slaves if any
	...
	double consumed_energy_at_period = new_energy - this->last_energy; // Energy consumed in this time period by me and my slaves
...
HostTemperature::HostTemperature(simgrid::s4u::Host* ptr) : host(ptr), last_updated(surf_get_clock()), slave_hosts()
{
	...
	if ((temperature_role_str != nullptr) && (!strcmp(temperature_role_str, "slave")) ) {
		// I'm just a slave, do nothing
		this->is_master = false;
		return;
	}
    // Else I'm a master (by default if no role was given)
	this->is_master = true;
 ...
 *  the value will be updated in kernel mode before returning the control to the requesting actor.
 */
double sg_host_get_air_temperature(const_sg_host_t host)
{
  xbt_assert(HostTemperature::EXTENSION_ID.valid(),
             "The Temperature plugin is not active. Please call sg_temperature_plugin_init() during initialization.");
 ...
 *  the value will be updated in kernel mode before returning the control to the requesting actor.
 */
double sg_host_get_outside_temperature(const_sg_host_t host)
{
  xbt_assert(HostTemperature::EXTENSION_ID.valid(),
             "The Temperature plugin is not active. Please call sg_temperature_plugin_init() during initialization.");
 ...
 *  the value will be updated in kernel mode before returning the control to the requesting actor.
 */
void sg_host_set_outside_temperature(const_sg_host_t host, double temperature)
{
  xbt_assert(HostTemperature::EXTENSION_ID.valid(),
             "The Temperature plugin is not active. Please call sg_temperature_plugin_init() during initialization.");
 ...
 */
void sg_host_init_master_slaves(const_sg_host_t host)
{
  xbt_assert(HostTemperature::EXTENSION_ID.valid(),
             "The Temperature plugin is not active. Please call sg_temperature_plugin_init() during initialization.");
